...
- Do not modify any files outside the folder created above 
  - exceptions:
    - app.agent.core.graph.build_graph
    - app.agent.core.utils.shared_store
    - app.agent.core.nodes.action.tool_enum
- Retrieve the information required for tool execution from:
  from app.agent.core.utils.shared_store import shared_store
- You must implement `def execute(self)`.
